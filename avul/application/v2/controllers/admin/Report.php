	...
	class Report extends CI_Controller {
		...
		public function attendance_report($param1="", $param2="", $param3="", $param4="", $param5="", $param6="", $param7="")
		{
        	...
        	if($param1 == 'excel_print')
        	{
			    ...
			    fputcsv($output, array('Employee Name','Store Name','Date', 'Clock In', 'Clock Out', 'Attendance Type'));
        	...
        	else
        	{
				...
				$page['pre_menu']     = "";
				$data['page_temp']    = $this->load->view('admin/report/attendance_report',$page,TRUE);
		...
		public function attendance_details_report($param1="", $param2="", $param3="", $param4="", $param5="", $param6="", $param7="")
		{
        	...
        	else
        	{
				...
				$page['pre_menu']     = "";
				$data['page_temp']    = $this->load->view('admin/report/attendance_details_report',$page,TRUE);
				$data['view_file']    = "Page_Template";
    	...
    	public function outlet_order($param1="", $param2="", $param3="", $param4="", $param5="", $param6="", $param7="")
    	{
			    ...
			    if(count($errors)==0)
			    {
			    	...
			    	if($start_date <= $end_date)
			    	{
				    	...
				    	if($data_list['status'] == 1)
				    	{
				    		...
				    		$excel_btn = '<a class="btn btn-success m-t-27" target="_blank" href="'.BASE_URL.'index.php/admin/report/outlet_order/excel_print/'.$start_date.'/'.$end_date.'/'.$state_val.'/'.$city_val.'/'.$zone_val.'/'.$order_val.'" style="color: #fff;"><i class="icon-grid"></i> Excel</a>';

				    		$pdf_btn   = '<a class="btn btn-danger m-t-27" target="_blank" href="'.BASE_URL.'index.php/admin/report/outlet_order/pdf_print/'.$start_date.'/'.$end_date.'/'.$state_val.'/'.$city_val.'/'.$zone_val.'/'.$order_val.'" style="color: #fff;"><i class="ft-file-text"></i> PDF</a>';
        	...
        	else
        	{
				...
				$page['pre_menu']     = "";
				$data['page_temp']    = $this->load->view('admin/report/outlet_order',$page,TRUE);
				$data['view_file']    = "Page_Template";
				$data['currentmenu']  = "outlet_order";
				$this->bassthaya->load_admin_form_template($data);
        	}
    	}
		public function outlet_stock($param1="", $param2="", $param3="", $param4="", $param5="", $param6="", $param7="")
    	{
    		if ($this->session->userdata('random_value') == '')
        	redirect(base_url() . 'index.php?login', 'refresh');
			    ...
			    if(count($errors)==0)
			    {
					...
					$type_id   = $this->input->post('type_id');
			    	    ...
			    	    $order_whr = array(
							...
							'type_id'    => $type_id,
							'outlet_id'  => $outlet_id,
					    	'method'     => '_StockOutletReport',
					    );

					    $data_list  = avul_call(API_URL.'report/api/stock_report',$order_whr);
				    	
				    	if($data_list['status'] == 1)
				    	{
							$data_details = $data_list['data'];
								
							$html     = '';

							foreach ($data_details as $key => $array) {
						
								$data_val=$data_details[$key];
								...
								foreach ($array as $key => $nestedArray) {
									$description = $nestedArray["description"];
									$company_name = $nestedArray["company_name"];
									$opening_sk = $nestedArray["opening_sk"];
									$emp_entry_sk = $nestedArray["emp_entry_sk"];
									$closeing_sk = $nestedArray["closeing_sk"];
									$pdt_unit = $nestedArray["pdt_unit"];
									$entry_date = $nestedArray["entry_date"];
									$sales = $nestedArray["sales"];
									$purchese = $nestedArray["purchese"];
									

									
									$purches_total += $purchese;
									$total_sales   += $sales;
									...
									$html .='
										<tr>
											<td>'.$num_no.'</td>
											<td>'.mb_strimwidth($company_name, 0, 35, '...').'</td>
											<td>'.$description.'</td>
											<td>'.$opening_sk.'</td>
											<td>'.$purchese.'</td>
											<td>'.$emp_entry_sk.'</td>
							...
							<tr>
								<th colspan="4" class="text-right">Purchaese Total</th>
								<td><b>'.$purches_total.'</b></td>
							
								<th colspan="1" class="text-right">Sales Total</th>
								<td><b>'.$sales.'</b></td>
								<th colspan="1" class="text-right">Stock</th>
								<td><b>'.$closeing_sk.'</b></td>
								<td></td>
							</tr>
							<tr> 
							<td></td>
						...
						//   $num++;
						    ...
						    $excel_btn = '<a class="btn btn-success m-t-27" target="_blank" href="'.BASE_URL.'index.php/admin/report/outlet_stock/excel_print/'.$month_id.'/'.$year_id.'/'.$outlet_id.'/'.$category_id.'/'.$sub_cat_id.'/'.$type_id.'" style="color: #fff;"><i class="icon-grid"></i> Excel</a>';

				    		$pdf_btn   = '<a class="btn btn-danger m-t-27" target="_blank" href="'.BASE_URL.'index.php/admin/report/outlet_order/pdf_print/'.$month_id.'/'.$year_id.'/'.$outlet_id.'/'.$category_id.'/'.$sub_cat_id.'/'.$type_id.'" style="color: #fff;"><i class="ft-file-text"></i> PDF</a>';
							$response['status']    = 1;
					        $response['message']   = $data_list['message']; 
					        $response['data']      = $html;
							$response['data_total'] = $html;
					        $response['excel_btn'] = $excel_btn;
					        $response['pdf_btn']   = $pdf_btn;
					        $response['error']     = []; 
					        echo json_encode($response);
			...
			}else if($param1 == 'sub_cat')
        	{
		        ...
		        return; 	
        	}else if($param1 == 'getSubProduct_list'){
			    ...
			    $product_list  = avul_call(API_URL.'catlog/api/product',$att_whr);
		    	
		    	$product_val  = $product_list['data'];
				
			
        		$option ='<option value="">Select product value</option>';

        		if(!empty($product_val))
        		{
        			foreach ($product_val as $key => $value) {
        				$id   = !empty($value['type_id']) ?$value['type_id']:'';
						$name =!empty($value['description'])?$value['description']:'';
        	...
        	if($param1 == 'excel_print')
        	{
				...
				$f_t_whr = array(
			    	'year_id' 	 => $year_id,
					'month_id'   => $month_id,
					'method'     => '_detail_days_Month',
				);
				$data_list_1  = avul_call(API_URL.'master/api/month',$f_t_whr);
				
		    		$start_date = $data_list_1['data_from'];
					$end_date = $data_list_1['data_to'];

			    header('Content-Type: text/csv; charset=utf-8');  
			    header('Content-Disposition: attachment; filename=outlet_order_report('.$start_date.' to '.$end_date.').csv');  
			    $output = fopen("php://output", "w");   
			    fputcsv($output, array('SI.NO','Entry Count','Outlet Name', 'Product Name','Entry Date', 'Stock Opening', 'Purchese Value', 'Employee Report Value', 'Sales Value', 'Unit Measure','Stock Closeing'));
			    ...
			    if($data_list['status'] == 1)
			    {
					...
					fputcsv($output, array('', '', '', '','','',$purches_total,'',$total_sales,'',$closeing_sk)); 
		    		}
			    }
        	...
        	else
        	{
				...
				$cat_data = avul_call(API_URL.'catlog/api/category',$where_4);
            	$cat_list = !empty($cat_data['data'])?$cat_data['data']:'';

		    	$page['method']       = '_getOutletStockData';
		    	$page['state_val']    = $state_list['data'];
				$page['month_list']   = $month_list;
				$page['cat_list']     = $cat_list;
				...
				$page['page_title']   = "Outlet Order Report";
				$page['pre_menu']     = "";
				$data['page_temp']    = $this->load->view('admin/report/outlet_stock',$page,TRUE);
				$data['view_file']    = "Page_Template";
				$data['currentmenu']  = "outlet_stock";
				$this->bassthaya->load_admin_form_template($data);
        	}
    	}
    	...
    	public function beat_outlet($param1="", $param2="", $param3="", $param4="", $param5="", $param6="", $param7="")
    	{
			...
			else
			{
				...
				$page['pre_menu']     = "";
				$data['page_temp']    = $this->load->view('admin/report/beat_outlet',$page,TRUE);
				$data['view_file']    = "Page_Template";
    	...
    	public function outlet_overall($param1="", $param2="", $param3="", $param4="", $param5="", $param6="", $param7="")
    	{
			...
			else
			{
				...
				$page['pre_menu']     = "";
				$data['page_temp']    = $this->load->view('admin/report/outlet_overall',$page,TRUE);
				$data['view_file']    = "Page_Template";
        ...
        public function outlet_history($param1="", $param2="", $param3="", $param4="", $param5="", $param6="", $param7="")
        {
			...
			else
			{
				...
				$page['pre_menu']     = "";
				$data['page_temp']    = $this->load->view('admin/report/outlet_history',$page,TRUE);
				$data['view_file']    = "Page_Template";
        ...
        public function order_report($param1="", $param2="", $param3="", $param4="", $param5="", $param6="", $param7="")
        {
        	...
        	else
			{
				...
				$page['pre_menu']     = "";
				$data['page_temp']    = $this->load->view('admin/report/order_report',$page,TRUE);
				$data['view_file']    = "Page_Template";
        ...
        public function target_achievement($param1="", $param2="", $param3="", $param4="", $param5="", $param6="", $param7="")
        {
			...
			if($param1 == 'emp_list')
        	{
			   ...
			   // value position_id like RSM,ASM ...
        	...
        	else
			{
				...
				$page['pre_menu']     = "";
				$data['page_temp']    = $this->load->view('admin/report/target_achievement',$page,TRUE);
				$data['view_file']    = "Page_Template";
        ...
        public function employee_order($param1="", $param2="", $param3="", $param4="", $param5="", $param6="", $param7="")
        {
        	...
        	else
        	{
				...
				$page['pre_menu']     = "";
				$data['page_temp']    = $this->load->view('admin/report/employee_order',$page,TRUE);
				$data['view_file']    = "Page_Template";
        ...
        { 
        	...
        	if($method =='_getProductStock')
        	{
			    ...
			    else
			    {
				    ...
				    {	
				    	...
				    	$excel_btn = '<a class="btn btn-success m-t-27" target="_blank" href="'.BASE_URL.'index.php/admin/report/product_stock/excel_print/'.$category_res.'/'.$sub_cat_res.'" style="color: #fff;"><i class="icon-grid"></i> Excel</a>';

			    		$pdf_btn   = '<a class="btn btn-danger m-t-27" target="_blank" href="'.BASE_URL.'index.php/admin/report/product_stock/pdf_print/'.$category_res.'/'.$sub_cat_res.'" style="color: #fff;"><i class="ft-file-text"></i> PDF</a>';
        	...
        	if($param1 =='excel_print')
        	{
				...
				if(!empty($sub_cat_id)){
					$sub_cat_val = explode('_', $sub_cat_id);
					$sub_cat_res = implode(',', $sub_cat_val);
				}else{
					$sub_cat_res ='';
				}
        ...
        public function distributor_stock($param1="", $param2="", $param3="", $param4="", $param5="", $param6="", $param7="")
        {
        	...
        	if($method =='_getDisProductStock')
        	{
			    ...
			    else
			    {
	            	...
	            	if($data_list['status'] == 1)
				    {
				    	...
				    	$excel_btn = '<a class="btn btn-success m-t-27" target="_blank" href="'.BASE_URL.'index.php/admin/report/distributor_stock/excel_print/'.$distributor_id.'/'.$category_res.'/'.$sub_cat_res.'" style="color: #fff;"><i class="icon-grid"></i> Excel</a>';

			    		$pdf_btn   = '<a class="btn btn-danger m-t-27" target="_blank" href="'.BASE_URL.'index.php/admin/report/distributor_stock/pdf_print/'.$distributor_id.'/'.$category_res.'/'.$sub_cat_res.'" style="color: #fff;"><i class="ft-file-text"></i> PDF</a>';
			...
			if($param1 =='getSubCategotry_list')
        	{
        		...
        		$s_category_list = avul_call(API_URL.'distributors/api/distributors',$where);
        		$s_category_res  = $s_category_list['data'];
				

        		$option ='<option value="">Select Value</option>';

        		if(!empty($s_category_res))
        		{
        			foreach ($s_category_res as $key => $value) {
        				$s_cat_id   = !empty($value['s_cat_id'])?$value['s_cat_id']:'';
                        $s_cat_name = !empty($value['s_cat_name'])?$value['s_cat_name']:'';

                        $option .= '<option value="'.$s_cat_id.'">'.$s_cat_name.'</option>';
        			}
        		}
        	...
        	if($param1 =='getCategotry_list')
        	{
				...
				// $sub_cat_res  = $category_list['data_sub'];
				...
				// $option_2 ='<option value="">Select Value</option>';

        		// if(!empty($sub_cat_res))
        		// {
        		// 	foreach ($sub_cat_res as $key => $value) {
        		// 		$s_cat_id   = !empty($value['s_cat_id'])?$value['s_cat_id']:'';
                //         $s_cat_name = !empty($value['s_cat_name'])?$value['s_cat_name']:'';

                //         $option_2 .= '<option value="'.$s_cat_id.'">'.$s_cat_name.'</option>';
        		// 	}
        		// }
        	...
        	if($param1 =='excel_print')
        	{
        		...
        		$category_val   = explode('_', $category_id);
        		$category_res   = implode(',', $category_val);
				$s_category_val   = explode('_', $sub_cat_id);
        		$s_category_res   = implode(',', $s_category_val);

        		$stock_whr = array(
        			'distributor_id' => $distributor_id,
        			'category_id'    => $category_res,
					's_cat_id'       => $s_category_res,
            		'method'         => '_overallDistributorStockReport',
            	);
    	...
    	public function employee_target($param1="", $param2="", $param3="", $param4="", $param5="", $param6="", $param7="")
    	{
        	...
        	else
			{
				...
				$data_list  = avul_call(API_URL.'employee/api/employee_designation',$where_r);
				$desgination_list   = !empty($data_list['data'])?$data_list['data']:'';
				$where_1 = array(
		    		'log_type' => '2',
		    		'method'   => '_typeWiseEmployee'
				...
				$page['pre_menu']     = "";
				$data['page_temp']    = $this->load->view('admin/report/employee_target',$page,TRUE);
				$data['view_file']    = "Page_Template";
        ...
        public function employee_report($param1="", $param2="", $param3="", $param4="", $param5="", $param6="", $param7="")
    	{
        	...
        	if($method =='_getEmployeeReport')
        	{
            	...
            	if($data_list['status'] == 1)
			    {
	                                        ...
	                                        foreach ($outlet_list as $key => $val_1) {
                            					...
                            					$html .= '
                            						<tr style="background-color: #f4f4f4;">
			                                            <td>'.$str_num.'</td>
			                                            <td colspan="5">'.$str_name.' - <b>Ph : </b> '.$mobile_val.'</td>
			                                        </tr>';
			                                        if($att_list)
			                                        {
			                                        	$html .= '
					...
					$att_id    = !empty($val_2['att_id'])?$val_2['att_id']:'';
					if($att_id){
						
					}
					$edit = '<a data-row="'.$att_id.'" data-value="admin" data-cntrl="report" data-func="employee_report" class="openModalBtn button_clr btn btn-primary"><i class="las la-image"></i> View </a>';
                    $html .= '
                    	<tr>
                            <td></td>
                            <td>'.$att_type.'</td>
                    ...
                    ';
			                                        ...
			                                        else
			                                        {
			                                        	$html .= '
			                                        		<tr>
					                                            <td colspan="6"><div class="alert alert-danger text-center"><b>No items found...</b></div></td>
					                                        </tr>
			                                        	';
			                                        }
                            					$str_num++;
		            ...
		            $excel_btn = '<a class="btn btn-success m-t-27" target="_blank" href="'.BASE_URL.'index.php/admin/report/employee_report/excel_print/'.$sel_date.'" style="color: #fff;"><i class="icon-grid"></i> Excel</a>';
			...
			if($param1 == 'get_img')
        	{
				...
				if($data_list){
					if($data_list['status'] == 1)
					{
						...
						foreach ($data_val as $value) {
						  ...
						  $outlet_img     = !empty($value['outlet_img'])?$value['outlet_img']:'---';
	
						  $image_urls[] = FILE_URL.'files/'.$outlet_img;
						}
			    ...
			    else
		    	{
					...
					$image_urls = array();
					$image_urls[]=BASE_URL.'app-assets/images/no_data_2.avif';
		    		$response['status']  = 0;
			        $response['data']    = $image_urls;
			        $response['error']   = []; 
			        echo json_encode($response);
        	...
        	else
			{
				...
				$page['pre_menu']     = "";
				$data['page_temp']    = $this->load->view('admin/report/employee_report',$page,TRUE);
				$data['view_file']    = "Page_Template";
