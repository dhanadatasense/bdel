	...
	class Outlets extends CI_Controller {
		...
		public function add_outlets($param1="", $param2="", $param3="")
		{
			...
			else
			{
				...
				$data['page_temp']    = $this->load->view('admin/outlets/add_outlets',$page,TRUE);
		...
		public function list_payment($param1="", $param2="", $param3="")
		{
			if ($this->session->userdata('random_value') == '')
        	redirect(base_url() . 'index.php?login', 'refresh');
        	...
        	if($param1 == 'payment_value')
			{
            	...
            	$where = array(
            		...
            		'limit'     => $limit,
            		'search'    => $search,
            		'outlet_id' => $random_val,
            		'method'    => '_listOutletPayment'
            	);

            	$data_list  = avul_call(API_URL.'outlets/api/outletPayment',$where);
            	$data_value = !empty($data_list['data'])?$data_list['data']:'';
            	...
            	if(!empty($data_value))
            	{
	            	...
	            	foreach ($data_value as $key => $value) {
					    ...
					    $table .= '
					    	<tr>
                                <td class="line_height">'.$i.'</td>
                                <td class="line_height">'.mb_strimwidth($distri_name, 0, 25, '...').'</td>
                                <td class="line_height">'.$pre_bal.'</td>
                                <td class="line_height">'.$amount.'</td>
                                <td class="line_height">'.$discount.'</td>
			...
			{	
				$page['main_heading'] = "Outlets";
				$page['sub_heading']  = "Outlets";
				$page['page_title']   = "List Payment";
				$page['pre_title']    = "List Outlet";
				$page['random_val']   = $param1;
				$page['pre_menu']     = "index.php/admin/outlets/list_outlets";
				$data['page_temp']    = $this->load->view('admin/outlets/list_payment',$page,TRUE);
				$data['view_file']    = "Page_Template";
				$data['currentmenu']  = "list_outlets";
				$this->bassthaya->load_admin_form_template($data);
			}
		...
		public function list_order_loyalty($param1="", $param2="", $param3="")
		{
			$method = $this->input->post('method');

			if($method == 'outlet_loyalty')
			{
				$error = FALSE;
				$invoice_count  = $this->input->post('invoice_count');
				$discount_value = $this->input->post('discount_value');
				$loyalty_id     = $this->input->post('loyalty_id');
				$outlets_id     = $this->input->post('random_val');
				$log_id         = $this->session->userdata('id');
				$log_role       = $this->session->userdata('user_role');

				$required = array('random_val');
				foreach ($required as $field) 
			    {
			        if(empty($this->input->post($field)))
			        {
			    ...
			    if(count(array_filter($invoice_count))!==count($invoice_count) || count(array_filter($discount_value))!==count($discount_value) || $error == TRUE)
			    {
			    	$response['status']  = 0;
			        $response['message'] = "Please fill all required fields"; 
			        $response['data']    = [];
			    ...
			    {
			    	if(userAccess('order-loyalty-add'))
			    	{
			    		$loyalty_val = [];
			    		$loyalty_num = count($invoice_count);

			    		for($j = 0; $j < $loyalty_num; $j++)
			    		{
			    			$loyalty_val[] = array(
			    				'invoice_count'  => $invoice_count[$j],
			    				'discount_value' => $discount_value[$j],
			    				'loyalty_id'     => $loyalty_id[$j],
			    			);
			    		}

			    		$loyalty_result = json_encode($loyalty_val);
			    		...
			    		$data = array(
					    	...
					    	'createdate'     => date('Y-m-d H:i:s'),
					    	'method'         => '_outletLoyalty',
					    );

					    $data_save = avul_call(API_URL.'outlets/api/outlet_loyalty',$data);
			...
			if($param1 == 'getOrderLoyalty_row')
			{
				$rowCount  = $this->input->post('rowCount');
				$outlet_id = $this->input->post('outlet_id');
				$newCount  = isset($rowCount)?$rowCount+1:'1';
				...
				$option = '
					<tr class="row_'.$newCount.'">
                        ...
                        <td style="padding-left: 0px;">
                        	<input type="text" data-te="'.$newCount.'" name="invoice_count[]" id="invoice_count'.$newCount.'" class="form-control invoice_count'.$newCount.' invoice_count" value="0" placeholder="Minimum Stock" onkeypress="return isNumberKey_new(event);">
                        	</td>
                        <td>
                        	<input type="text" data-te="'.$newCount.'" name="discount_value[]" id="discount_value'.$newCount.'" class="form-control discount_value'.$newCount.' discount_value" value="0" placeholder="Minimum Stock" onkeypress="return isNumberKey_new(event);">
                        	<input type="hidden" data-te="'.$newCount.'" name="loyalty_id[]" id="loyalty_id'.$newCount.'" class="form-control loyalty_id'.$newCount.' loyalty_id" value="0" placeholder="Minimum Stock">
                        </td>
                        <td><button type="button" name="remove" class="btn btn-danger btn-sm  button_size m-t-6 remove_item"><span class="ft-minus-square"></span></button></td>
                    </tr>
				';
			...
			{
				$whr_1 = array('outlet_id' => $param1, 'method' => '_outletLoyaltyList');
	            $val_1 = avul_call(API_URL.'outlets/api/outlet_loyalty',$whr_1);
	            $res_1 = $val_1['data'];

	            $page['data_value']   = $res_1;
				$page['main_heading'] = "Outlets";
				$page['sub_heading']  = "Outlets";
				$page['page_title']   = "Order Loyalty";
				$page['pre_title']    = "List Outlet";
				$page['random_val']   = $param1;
				$page['pre_menu']     = "index.php/admin/outlets/list_outlets";
				$data['page_temp']    = $this->load->view('admin/outlets/list_order_loyalty',$page,TRUE);
				$data['view_file']    = "Page_Template";
				$data['currentmenu']  = "list_outlets";
				$this->bassthaya->load_admin_form_template($data);
			}
		...
		public function new_outlets($param1="", $param2="", $param3="")
		{
			if ($this->session->userdata('random_value') == '')
        	redirect(base_url() . 'index.php?login', 'refresh');
			...
			if($param1 == '')
			{
				$page['main_heading'] = "Outlets";
				$page['sub_heading']  = "Outlets";
				$page['page_title']   = "Today Outlets";
				$page['pre_title']    = "Add Outlets";
				$page['page_access']  = userAccess('outlets-add');
				$page['pre_menu']     = "index.php/admin/outlets/add_outlets";
				$data['page_temp']    = $this->load->view('admin/outlets/new_outlets',$page,TRUE);
				$data['view_file']    = "Page_Template";
				$data['currentmenu']  = "list_outlets";
				$this->bassthaya->load_admin_form_template($data);
			}
		...
		public function balance_list($param1="", $param2="", $param3="")
		{
			if ($this->session->userdata('random_value') == '')
        	redirect(base_url() . 'index.php?login', 'refresh');
			...
			if($param1 == '')
			{
				$page['main_heading'] = "Outlets";
				$page['sub_heading']  = "Outlets";
				$page['page_title']   = "Outlets Balance";
				$page['pre_title']    = "Add Outlets";
				$page['page_access']  = userAccess('outlets-add');
				$page['pre_menu']     = "index.php/admin/outlets/add_outlets";
				$data['page_temp']    = $this->load->view('admin/outlets/balance_list',$page,TRUE);
				$data['view_file']    = "Page_Template";
				$data['currentmenu']  = "list_outlets";
				$this->bassthaya->load_admin_form_template($data);
			}
				...
				if(userAccess('outlets-view'))
				{
	            	...
	            	if(!empty($data_value))
	            	{
		            	...
		            	foreach ($data_value as $key => $value) {
							...
							$sales_type = !empty($value['sales_type'])?$value['sales_type']:'';

						    if($sales_type == '1')
			                {
			                	$status_view = '<a data-row="'.$i.'" data-id="'.$outlet_id.'" data-status="'.$sales_type.'" data-value="admin" data-cntrl="outlets" data-func="balance_list" class="list_'.$i.' enable_btn_'.$i.' status-btn button_clr btn btn-success"><i class="ft-edit"></i> Enable </a>';
			                }
			                else
			                {
			                	$status_view = '<a data-row="'.$i.'" data-id="'.$outlet_id.'" data-status="'.$sales_type.'" data-value="admin" data-cntrl="outlets" data-func="balance_list" class="list_'.$i.' disable_btn_'.$i.' status-btn button_clr btn btn-danger"><i class="ft-edit"></i> Disable </a>';
			                }

						    $table .= '
						    	<tr class="row_'.$i.'">
	                                <td class="line_height">'.$i.'</td>
	                                <td class="line_height">'.mb_strimwidth($dis_code, 0, 10, '...').'</td>
	                                <td class="line_height">'.mb_strimwidth($store_name, 0, 25, '...').'</td>
	                                <td class="line_height">'.$bill_no.'</td>
	                                <td class="line_height">'.$bal_amt.'</td>
	                                <td class="line_height">'.$date.'</td>
	                                <td class="line_height">'.$status_view.'</td>
	                            </tr>
						    ';
			...
			else if($param1 == 'status_update')
			{
				if(userAccess('outlets-edit'))
				{
					...
					if(!empty($id))	
					{
						...
						$data_save = avul_call(API_URL.'outlets/api/outletPayment',$upt_val);
